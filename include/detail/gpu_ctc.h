...
ctcStatus_t GpuCTC<ProbT>::launch_alpha_beta_kernels(const ProbT* const probs,
    ...
    if (compute_alpha)
        hipLaunchKernelGGL((compute_alpha_kernel<ProbT, NT, VT>), dim3(grid_size), dim3(NT), 0, stream_, probs, label_sizes_, utt_length_,
    ...
    if (compute_beta) {
        hipLaunchKernelGGL((compute_betas_and_grad_kernel<ProbT, NT, VT>), dim3(grid_size), dim3(NT), 0, stream_, probs, label_sizes_, utt_length_, repeats_,
